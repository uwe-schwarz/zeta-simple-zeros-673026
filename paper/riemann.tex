...
finite certificate in Ainta's AI-assisted draft~\cite{Ainta26}.  Keeping that
draft's stability, overlap-kernel, block-aggregation, and pinching arguments,
which refine the rank--trace argument of Claude~\cite{Claude26}, and adding an
exact finite-dimensional spectral envelope plus a rigorously reproduced
stronger interval certificate gives
\[
...
not as a completed formal-verification claim.
\end{abstract}
...
The diagonal entries are $1+o(1)$; normalizing the diagonal produces a
correlation matrix that differs from $G_B$ by $o(1)$ in Frobenius norm.
Since $m$ is fixed and $\Psi$ is globally $2$-Lipschitz, this changes both
the defect and the off-diagonal energy by $o(1)$.  Lemma~\ref{lem:block-energy},
Lemma~\ref{lem:spectral-envelope}, and \eqref{eq:span-tradeoff} therefore give
uniformly in $B$
...
value between decimals ending in $\ldots0978$ and $\ldots0979$.  An exact
scan through $7\le m\le1000$ gives a unique maximum at $m=279$, with
...
a Lean certificate consumer.  At the snapshot date, the generic MVT,
segment, replay, and scaled-checker semantics and the complete code-9 catalog
have been kernel-built; the code-10 production catalog is in progress, and
the full code-2 catalog and final dependency-closed public build are deferred.
...
public draft~\cite{Ainta26}; the stability, overlap-kernel, block-aggregation,
and shifted-pinching arguments originate there.  The new contribution in this
revision is the exact finite-dimensional spectral envelope, the stronger exact
finite certificate and its reproducibility package, and the partially
completed Lean certificate-consumer effort.  The clean-room
reproduction~\cite{Learademacher26} provides a separate comparison point.
...
AI-assisted research draft and verifier artifact, August 2026,
\href{https://github.com/ainta/zeta-simple-zeros}
{GitHub repository}.

\bibitem{Learademacher26}
L.~Rademacher,
\emph{AI refines an AI-derived zeta bound},
clean-room research draft and reproduction artifact, August 2026,
\href{https://github.com/learademacher/ai-refines-ai-zeta-bound}
{GitHub repository}.
